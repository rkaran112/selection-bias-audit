...
Built on the two LendingClub files: **{{n_accepted_total}} funded loans** with
repayment outcomes, and **{{n_rejected_total}} declined applications** with
...
| Problem | Scale |
| --- | --- |
| Declines with **no bureau score at all** | **{{unscored_pct}}** — and concentrated in later vintages (93% missing in 2018) |
| Scored declines reaching the 1st percentile of the approved book | **{{overlap_pct}}%** |
| Approved loans with no settled outcome yet (dropped) | {{indeterminate_pct}} |
| Platform approval rate | {{accept_rate}} |
...
applications ({{n_rejected_used}} of {{n_rejected_sampled}} sampled). **The
...
| | |
| --- | --- |
| Reported Gini (held-out, approved loans only) | **{{base_gini}}** |
| KS / AUC | {{base_ks}} / {{base_auc}} |
| Observed bad rate on approved loans | {{obs_bad_rate}} |
| Estimated through-the-door bad rate | **{{ttd_range}}** depending on method |
| Heckman selection correlation ρ | **{{rho}}** (se {{rho_se}}, LR p = {{rho_p}}) |
| Least-biased method in simulation | **{{best_method}}** ({{best_bias}} pp mean abs. error) |
| Where it stops working | {{verdict}} |
| Methods that fail the null case | **{{hallucinators}}** |
| Residual ZIP dispersion ratio | {{disp_ratio}} against 1.0 under the null |
...
selection correlation ρ is **{{rho}}** (se {{rho_se}}, LR p = {{rho_p}}) with no
exclusion restriction, and **{{rho_x}}** (LR p = {{rho_xp}}) when application
year is excluded from the outcome equation. {{rho_verdict}}
...
size.** With {{n_modelled}} applicants,
a correlation of {{rho_x}} is distinguishable from zero and still far too small
...
{{scale_finding}} But the multiple that works differs by regime — 1.0 under the
...
declined — has an inferred bad rate of {{swapin_range}}. That has to be judged
against the break-even those particular applicants face, **{{swapin_breakeven}}**,
not against the pooled {{breakeven}}: the swap-in set lands in the safest score
...
All four methods agree on the sign: **{{profit_verdict}}** The sign is also
stable across the funding-cost sensitivity grid — only the magnitude moves,
from {{profit_lo_sens}} to {{profit_hi_sens}}. This contradicts the premise the
...
- **The swap set is not a list of people to approve.** It is large mostly
  ...
  because good business is sitting in the decline bin. {{swapin_profile}} That
...
- **The profit figure is assumption-sensitive in magnitude.** Yields, losses
  ...
  estimate moves from {{profit_lo_sens}} to {{profit_hi_sens}} — a sevenfold
...
- **It does not transport to your portfolio unexamined.** LendingClub approved
  about {{accept_rate}} of applications. A lender approving 60% has a far
...
Seed `{{seed}}`, every dependency pinned, every sample drawn from a seeded
...
Runtime is roughly {{runtime}} on a laptop, dominated by the Part 3 sweep.
...
binning, logistic regression, scaled to points ({{pdo}} points to double the
...
| **Inverse propensity** | approval is uninformative about default given the shared fields | a declined applicant must plausibly have been approvable; here that positivity condition fails badly (approval AUC {{prop_auc}}) |
...
Swept across rejection rates of {{rates}}, {{n_reps}} replicates each, under
